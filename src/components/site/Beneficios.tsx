    ...
    <section className="bg-background py-28 md:py-36">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="grid gap-16 lg:grid-cols-[0.9fr_1.1fr] lg:gap-24">
          ...
          <div className="grid gap-px border-t border-l border-border sm:grid-cols-2">
            {benefits.map((benefit, index) => (
              <Reveal
                key={benefit.title}
                delay={index * 60}
                className="border-r border-b border-border p-7 transition-colors duration-500 hover:bg-surface"
              >
                <h3 className="text-sm font-semibold tracking-wide text-ink">{benefit.title}</h3>
